...
class ZarDomainBackupCodec {
  ...
  ZarDomainBackupBundle decodeJson(String source) {
    ...
    final deals = _mapList(
      raw['deals'],
      (map) => _dealFromMap(map, businessId),
    );
  ...
  ZarDeal _dealFromMap(Map<String, Object?> map, String businessId) {
    var amount = ZarAssetAmount.fromMap(
      _requiredMap(map['amount'], 'deal.amount'),
    );
    ...
    return ZarDeal(
      ...
      businessId: businessId,
      type: ZarDealType.values.byName(
        _requiredString(map['type'], 'deal.type'),
      ),
  ...
  Map<String, Object?> _settlementToMap(ZarSettlement settlement) => {
    ...
    'coinValuation': settlement.coinValuation?.toMap(),
  ...
  ZarSettlement _settlementFromMap(
    ...
    coinValuation: map['coinValuation'] == null
        ? null
        : ZarCoinSettlementValuation.fromMap(
            _requiredMap(map['coinValuation'], 'settlement.coinValuation'),
          ),
  ...
  void _validateReferences({
    ...
    for (final deal in deals) {
      ...
      if (requireCoinCatalog && deal.amount is ZarCoinBundleAmount) {
        for (final line in (deal.amount as ZarCoinBundleAmount).lines) {
          if (!coinTypeIds.contains(line.coinTypeId)) {
            throw FormatException(
    ...
    for (final settlement in settlements) {
      ...
      if (requireCoinCatalog && settlement.amount is ZarCoinBundleAmount) {
