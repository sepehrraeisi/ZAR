...
class _PhaseA2ShellState extends State<PhaseA2Shell> {
  ...
  late final List<AppRecord> _records = [
    AppRecord(
      id: 's1', type: RecordType.settlement, operationLabel: 'تحویل', personId: 'p2',
      amountDisplay: r'$10,000', assetLabel: 'ارز', currencyCode: 'USD',
      date: Jalali.now().addDays(-1), time: const TimeOfDay(hour: 11, minute: 0),
    ),
    AppRecord(
      id: 's2', type: RecordType.settlement, operationLabel: 'دریافت', personId: 'p1',
      amountDisplay: '۲۵۰', assetLabel: 'گرم طلا', date: Jalali.now(),
      time: const TimeOfDay(hour: 10, minute: 30),
    ),
    AppRecord(
      id: 's3', type: RecordType.settlement, operationLabel: 'تحویل', personId: 'p3',
      amountDisplay: '€5,000', assetLabel: 'ارز', currencyCode: 'EUR',
      date: Jalali.now(), time: const TimeOfDay(hour: 14, minute: 45),
    ),
    AppRecord(
      id: 's4', type: RecordType.settlement, operationLabel: 'دریافت', personId: 'p4',
      amountDisplay: '۴۰۰', assetLabel: 'گرم طلا', date: Jalali.now().addDays(1),
    ),
    AppRecord(
      id: 's5', type: RecordType.settlement, operationLabel: 'تحویل', personId: 'p2',
      amountDisplay: r'$8,000', assetLabel: 'ارز', currencyCode: 'USD',
      date: Jalali.now().addDays(-2), time: const TimeOfDay(hour: 12, minute: 20),
  ...
  List<AppRecord> get historyRecords => _records.where((r) => r.type == RecordType.settlement && r.status != SettlementStatus.open).toList(growable: false);

  int openCountFor(String personId) =>
      _records.where((r) => r.personId == personId && r.isObligation && r.status == SettlementStatus.open).length;
  ...
  Future<void> _archivePerson(AppPerson person) async {
    final shouldArchive = await confirmArchiveWithOpenObligations(
      context,
      openObligations: openCountFor(person.id),
    );
        ...
        builder: (_) => ArchivedPeopleScreen(
          people: archivedPeople
              .map((p) => ArchivedPersonViewData(
                    ...
                    name: p.name,
                    phone: p.phone,
                    openObligations: openCountFor(p.id),
                  ))
