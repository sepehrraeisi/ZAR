...
void main() {
  ...
  test('groups completed movements by currency and gold fineness', () {
    ...
    expect(
      value.payableAssetBuckets.map((item) => item.currencyCode),
      containsAll(<String?>['USD', 'EUR']),
    );
    expect(
      value.payableAssetBuckets.map((item) => item.goldFineness),
      containsAll(<String?>['750', '740']),
    );
    expect(value.payableAssetBuckets, hasLength(4));
  });

  test('keeps coin variants separate and ignores open settlements', () {
    ...
    expect(value.payableAssetBuckets.single.displayName, contains('سکه امامی'));
    expect(value.payableAssetBuckets.single.amount, '2');
  });
}
